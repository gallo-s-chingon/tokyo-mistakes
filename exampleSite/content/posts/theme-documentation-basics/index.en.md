...
draft: false
author: "Gallo Chingo with assistance from Claude Sonnet 4.5"
authorLink: "https://schingon.com"
description: "Discover what the Hugo - Tokyo-Nord theme is all about and the core-concepts behind it."
images: []
resources:
- name: "featured-image"
...
Discover what the Hugo - **Tokyo-Nord** theme is all about and the core-concepts behind it.
...
The **Tokyo-Nord** theme's repository is: [https://github.com/gallo-s-chingon/Tokyo-Nord](https://github.com/gallo-s-chingon/Tokyo-Nord).

You can download the [release :(far fa-file-archive fa-fw): .zip file](https://github.com/gallo-s-chingon/Tokyo-Nord/releases) of the theme and extract it in the `themes` directory.
...
{{< admonition note "Tokyo-Nord theme's compatibility" >}}

| Tokyo-Nord branch or version | Supported Hugo versions |
|:-------------------------|:-----------------------:|
...
The following is a basic configuration for the Tokyo-Nord theme:
...
[markup]
  ...
  [markup.highlight]
    # false is a necessary configuration (https://github.com/gallo-s-chingon/Tokyo-Nord/issues/158)
    noClasses = false
```
...
In addition to [Hugo global configuration](https://gohugo.io/overview/configuration/) and [menu configuration](#basic-configuration), **Tokyo-Nord** lets you define the following parameters in your site configuration (here is a `hugo.toml`, whose values are default).
...
# Author config
  ...
  [params.page]
    ...
    [params.page.comment]
      ...
      [params.page.comment.valine]
        ...
        # located in "themes/Tokyo-Nord/assets/lib/valine/emoji/" directory
  ...
  [params.cdn]
    ...
    # ["jsdelivr.yml"]
    # located in "themes/Tokyo-Nord/assets/data/cdn/" directory
    # you can store your own data files in the same path under your project:
    # "assets/data/cdn/"
    data = ""
...
[markup]
  ...
  [markup.highlight]
    ...
    # ({{< link "https://github.com/gallo-s-chingon/Tokyo-Nord/issues/158" >}})
    noClasses = false
...
The default CDN data file is located in `themes/Tokyo-Nord/assets/data/cdn/` directory.
...
The default data of all supported social links is located in `themes/Tokyo-Nord/assets/data/social.yaml`,
...
**Tokyo-Nord** theme has been built to be as configurable as possible by defining custom `.scss` style files.

The directory including the custom `.scss` style files is `assets/css` relative to **your project root directory**.

In `assets/css/_override.scss`, you can override the variables in `themes/Tokyo-Nord/assets/css/_variables.scss` to customize the style.
...
**Tokyo-Nord** theme is fully compatible with Hugo multilingual mode, which provides in-browser language switching.
...
To override these values, create a new file in your local i18n folder `i18n/<languageCode>.toml` and inspire yourself from `themes/Tokyo-Nord/i18n/en.toml`.

By the way, as these translations could be used by other people, please take the time to propose a translation by [:(fas fa-code-branch fa-fw): making a PR](https://github.com/gallo-s-chingon/Tokyo-Nord/pulls) to the theme!

## 5 Search

{{< version 0.2.0 >}}

Based on [Lunr.js](https://lunrjs.com/) or [algolia](https://www.algolia.com/), searching is supported in **Tokyo-Nord** theme.
