...
var Theme = /*#__PURE__*/function () {
  function Theme() {
    ...
    // Theme mapping based on contrast setting
    this.highContrast = this.config.highContrast !== false; // default to true
    this.themeMap = {
      dark: this.highContrast ? 'tokyo-night' : 'nordic',
      light: this.highContrast ? 'onenord-light' : 'onenord-day'
    };

    this.currentTheme = document.body.getAttribute('data-theme') || this.themeMap.dark;
    this.isDark = this.currentTheme === 'tokyo-night' || this.currentTheme === 'nordic';
    ...
    value: function initSwitchTheme() {
      ...
      Util.forEach(document.getElementsByClassName('theme-switch'), function ($themeSwitch) {
        $themeSwitch.addEventListener('click', function () {
          ...
          // Uses contrast-aware theme mapping
          var newTheme = _this2.isDark ? _this2.themeMap.light : _this2.themeMap.dark;

          _this2.currentTheme = newTheme;
      ...
      if (this.config.comment) {
        if (this.config.comment.utterances) {
          ...
          this._utterancesOnSwitchTheme = this._utterancesOnSwitchTheme || function () {
            var message = {
              type: 'set-theme',
              theme: _this6.isDark ? utterancesConfig.darkTheme : utterancesConfig.lightTheme
            };
        ...
        if (this.config.comment.giscus) {
          ...
          this._giscusOnSwitchTheme = this._giscusOnSwitchTheme || function () {
            var message = {
              setConfig: {
                theme: _this6.isDark ? giscusConfig.darkTheme : giscusConfig.lightTheme,
